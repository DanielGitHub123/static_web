...
$(document).ready(function(){
	...
	var getColumnClass = function(cls){
		return cls.replace('task-list','').replace('ui-sortable','').replace(/\s/g,'');
	};
	...
	$('.task-list').sortable({
		...
		receive : function(e,ui){
			...
			if(rIndex != trIndex){
				...
				$($(ui.item.get(0)).data('ddContainer') || '.task-list').sortable('cancel');
				return false;
			}else{
	...
	//通过下拉菜单改变状态
	$('.task-list').delegate('a[data-handler=waiting],a[data-handler=devping],a[data-handler=done]','click',function(e) {
		var tdClassName = $(this).attr('data-handler'),coClassName = $(this).closest('td').attr('class'),
			columnClass = coClassName.replace('task-list','').replace('ui-sortable','').replace(/\s/g,'');
		var cnMsg = {'waiting':'待开发','devping':'开发中','done':'已完成'};
		if(tdClassName == columnClass){
			...
			$(this).closest('tr').children('td.'+ tdClassName).append($(this).closest('div.task-item').remove());
			$.globalMessenger().post({
